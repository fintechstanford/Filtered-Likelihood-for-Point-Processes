##############################################################################################################
### This code computes an approximation of the filtered paths of the intensity, the partially observed factor Y, and the frailty Z of the paper "Filtered Likelihood for Point Processes" by Kay Giesecke and Gustavo Schwenkler. The copyright to these codes remains with the authors.
##############################################################################################################
...
ep = read.table("simulated_events_2.txt", header = FALSE, skip = 1)
exp_f = read.table("simulated_factors_2.txt", header = FALSE, skip = 1)
...
### The filtered log-likelihood is computed by these functions ###

# Approximate the path of the filtered intensity, the filtered partially observed factor Y, and the filtered frailty Z. 
fi = filtered_intensity(par, tau2)
fY = filtered_Y(par, tau2)

# Store the filtered paths in different document:
write(c("Theta = ", par), "filtered_intensity.txt", ncolumns = length(par)+1, sep = "\t")
write(fi, "filtered_intensity.txt", ncolumns = 1, append = TRUE)

write(c("Theta = ", par), "filtered_Y.txt", ncolumns = length(par)+1, sep = "\t")
write(fY, "filtered_Y.txt", ncolumns = 1, append = TRUE)
