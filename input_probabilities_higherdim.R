##############################################################################################################
### Computation of the probabilities hat p of Section 4 of the paper "Filtered Likelihood for Point Processes" by Kay Giesecke and Gustavo Schwenkler in the higher dimensional case. The copyright to these codes remains with the authors. ###
	...
	if ((length(from_obs) > 1) & (length(to_vec) > 1)) {
		...
		for (j in 1:J) {
			...
			if (tildeY0 < 0) {
				...
				land_in = ifelse(land_in == 0, 1, land_in)
